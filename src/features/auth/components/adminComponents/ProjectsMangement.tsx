...
export default function ProjectsManagement({
  ...
  description = "Manage and track all user tenders",
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800 transition-colors">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}>
                    {[40, 80, 100, 100, 100, 100, 90, 80].map((w, j) => (
                ...
                <tr>
                  <td colSpan={8} className="text-center py-16 text-slate-400 dark:text-gray-500 text-sm">
                    {search ? `No tenders match "${search}".` : "No tenders found."}
                  </td>
                </tr>
              ) : (
                projects.map((project, idx) => (
                  ...
                  >
                    ...
                    <td className=" px-6 py-5 text-sm font-semibold text-gray-900 dark:text-white transition-colors max-w-[200px]">
                      {project.name}
                    </td>
                    ...
                    <td className="whitespace-nowrap px-6 py-5 text-sm font-medium text-gray-500 dark:text-gray-400 transition-colors">
                      {project.questionDate ? new Date(project.questionDate).toLocaleDateString() : "N/A"}
                    </td>
                    <td className="whitespace-nowrap px-6 py-5  text-sm font-medium text-gray-500 dark:text-gray-400 transition-colors">
                      {project.closingDate ? new Date(project.closingDate).toLocaleDateString() : "N/A"}
                    </td>
        ...
        {selectedProject && (
          <div className="mt-4 space-y-6 text-sm text-gray-700 dark:text-gray-300">
            <div className="grid grid-cols-2 gap-y-4 gap-x-6">
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Tender Name</span>
              ...
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Question Due Date</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{new Date(selectedProject.questionDate).toLocaleDateString()}</span>
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Closing Date</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{new Date(selectedProject.closingDate).toLocaleDateString()}</span>
              </div>
