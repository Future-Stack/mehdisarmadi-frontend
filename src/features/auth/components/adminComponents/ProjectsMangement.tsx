...
// import { useGetAdminProjectsQuery } from "@/store/api/projectApi";
...
//                       {(page - 1) * limit + idx + 1}
//                     </td>
//                     <td className="whitespace-nowrap px-6 py-5 text-sm font-semibold text-gray-900 dark:text-white transition-colors truncate max-w-[200px]">
//                       {project.name}
//                     </td>
...
//               <div><span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Owner</span><span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.owner?.fullName || "Unknown"}</span></div>
//               <div><span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Client</span><span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.clientName || "Unknown"}</span></div>
...
import StaticPage from "@/components/layout/StaticDemoPage";
import { Modal } from "@/components/ui/Modal";
import { useGetAdminProjectsQuery, AdminProjectListItem } from "@/store/api/projectApi";
...
function StatusBadge({ status }: { status: ProjectStatus }) {
  ...
  if (s === "completed" || s === "done") {
    styles = "bg-[#DDFFEB] dark:bg-[#0082361A] text-[#008236]";
  } else if (s === "needsreview" || s === "pending") {
    styles = "bg-[#FFFADA] dark:bg-[#92400E1A] text-[#92400E]";
  } else if (s === "failed" || s === "error") {
    styles = "bg-[#FEE2E2] dark:bg-[#E7000B1A] text-[#991B1B]";
  }
          ...
          <table className="w-full text-left">
              ...
              <tr className="border-b border-gray-100 dark:border-gray-800 bg-slate-50/60 dark:bg-gray-900/50 transition-colors">
                {TABLE_HEADINGS.map((heading) => (
                  <th
                    key={heading}
                    scope="col"
                    className="whitespace-nowrap px-6 py-4 text-left text-[12px] font-bold uppercase tracking-tight text-[#4A5565] dark:text-gray-500 max-w-[200px] truncate transition-colors"
                  >
                    {heading}
                  </th>
                ))}
            ...
            <tbody className="divide-y divide-gray-50 dark:divide-gray-800 transition-colors">
              {isLoading ? (
                Array.from({ length: 5 }).map((_, i) => (
                  <tr key={i}>
                    {[40, 140, 100, 80, 100, 100, 60].map((w, j) => (
                      <td key={j} className="px-6 py-5">
                        <div className="h-3.5 rounded-full bg-slate-100 dark:bg-gray-800 animate-pulse" style={{ width: w }} />
                      </td>
                    ))}
                ...
                <tr>
                  <td colSpan={7} className="text-center py-16 text-slate-400 dark:text-gray-500 text-sm">
                    {search ? `No projects match "${search}".` : "No projects found."}
                  </td>
                </tr>
              ) : (
                projects.map((project, idx) => (
                  ...
                  >
                    ...
                    <td className="whitespace-nowrap px-6 py-5 text-center text-sm font-medium text-gray-500 dark:text-gray-400 transition-colors">
                      {String(project.fileCount || 0).padStart(2, "0")}
                    </td>
                    <td className="whitespace-nowrap px-6 py-5">
                      <StatusBadge status={project.statusLabel || project.status} />
                    </td>
      ...
      >
        {selectedProject && (
          <div className="mt-4 space-y-6 text-sm text-gray-700 dark:text-gray-300">
            <div className="grid grid-cols-2 gap-y-4 gap-x-6">
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Project Name</span>
                <span className="font-semibold text-gray-900 dark:text-white transition-colors">{selectedProject.name}</span>
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Owner</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.owner?.fullName || "Unknown"}</span>
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Client</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.clientName || "Unknown"}</span>
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Files count</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.fileCount || 0}</span>
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-2">Status</span>
                <StatusBadge status={selectedProject.statusLabel || selectedProject.status} />
              </div>
              <div>
                <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Created Date</span>
                <span className="font-medium text-gray-900 dark:text-white transition-colors">{new Date(selectedProject.createdAt).toLocaleDateString()}</span>
              </div>

              {selectedProject.address && (
                <div className="col-span-2">
                  <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Address</span>
                  <span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.address}</span>
                </div>
              )}
              {selectedProject.description && (
                <div className="col-span-2">
                  <span className="block text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-1">Description</span>
                  <span className="font-medium text-gray-900 dark:text-white transition-colors">{selectedProject.description}</span>
                </div>
              )}
            </div>
