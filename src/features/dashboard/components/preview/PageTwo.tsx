...
export const PageTwo = ({ exportMode, quoteData }: { exportMode?: boolean; quoteData?: any }) => {
    ...
    <div className={exportMode ? "bg-white text-gray-900 font-sans " : "bg-white text-gray-900 shadow-xl border border-gray-200 mx-auto w-full max-w-[850px] min-h-[1100px]  flex flex-col font-sans"}>
        {!exportMode && <DocumentHeader showFull={false} quoteData={quoteData} />}
        ...
        <div className={`mb-8 px-12 ${exportMode ? 'export-section' : ''}`}>
            ...
            {q.separatePrices && Array.isArray(q.separatePrices) && q.separatePrices.length > 0 ? (
                q.separatePrices.map((sp: any, i: number) => (
                    <div key={i} className="border border-[#EDEDED] rounded-xl p-5 mb-4 bg-gray-50/40">
                        <div className="flex justify-between items-start mb-3 pb-2.5 border-b border-[#EFEFEF]">
                            <div className="flex gap-2 items-center">
                                <span className="text-[9px] font-bold text-emerald-700 bg-emerald-100 border border-emerald-200 px-2 py-0.5 rounded uppercase">{sp.id || `SP-0${i + 1}`}</span>
                                <span className="text-[12px] font-bold text-gray-900">{sp.title}</span>
                            </div>
                            <div className="text-[13px] font-black text-gray-900">{sp.price}</div>
                        </div>
                ...
                <div className="border border-[#EDEDED] rounded-xl p-5 mb-4 bg-gray-50/40">
        ...
        <div className={`flex-1 px-12 ${exportMode ? 'export-section' : ''}`}>
            ...
            {q.altPrices && Array.isArray(q.altPrices) && q.altPrices.length > 0 ? (
                q.altPrices.map((ap: any, i: number) => (
                    <div key={i} className="border border-[#EDEDED] rounded-xl p-5 bg-gray-50/40 mb-4">
                        <div className="flex justify-between items-start mb-3 pb-2.5 border-b border-gray-200">
                            <div className="flex gap-2 items-center">
                                <span className="text-[9px] font-bold text-blue-700 bg-blue-100 border border-blue-200 px-2 py-0.5 rounded uppercase">{ap.id || `ALT-0${i + 1}`}</span>
                                <span className="text-[12px] font-bold text-gray-900">{ap.title}</span>
                            </div>
                            <div className="text-[13px] font-black text-gray-900">{ap.price}</div>
                        </div>
                        <div className="text-[11px] text-gray-600 space-y-3">
                            <div>
                                <div className="font-bold text-gray-800 mb-1.5">Description</div>
                ...
                <div className="border border-[#EDEDED] rounded-xl p-5 bg-gray-50/40">
                    <div className="flex justify-between items-start mb-3 pb-2.5 border-b border-gray-200">
                        <div className="flex gap-2 items-center">
